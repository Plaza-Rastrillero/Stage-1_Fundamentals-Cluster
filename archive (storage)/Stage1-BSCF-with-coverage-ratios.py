...
ARCHIVED - retired, imported by nothing, kept only for one reason.

This is the earlier sibling of Stage1-BSCF.py in this folder, and the two are
not interchangeable. That one is later: it gained the doubt gate
(DOUBT_ABSOLUTE_MAX, DOUBT_VS_SIGNAL_MAX) that now lives in bscflib/formula.py.
This one predates the removal and still carries what that removal took out -
`debt_coverage` and `interest_coverage`, built on operating cash flow and
interest expense, and the year-to-date differencing quarterizer that turns XBRL
duration facts into TTM windows.

That machinery exists in no other file and in no other commit. If leverage and
the coverage ratios are ever added back to the live tool, this is the reference
implementation to read. Do not delete it on the assumption that the other copy
is the same file.

Usage (historical - this is not the entry point any more; run bscf.py):
...
def analyze_ticker(
    ...
    current_debt = resolve_instant(index, "current_lt_debt", latest["as_of"]) or 0.0
    ...
    return {
        ...
        "ocf_basis": ocf_basis,
        "debt_coverage": (current_debt / ocf) if ocf else None,
    ...
    rows = [
        {
            ...
            "debt_cov": fmt_ratio(r["debt_coverage"], 2),
            "int_cov": fmt_ratio(r["interest_coverage"], 2),
            "gaps": r["gaps"] or "-",
        }
        for r in results
    ]
...
def main() -> int:
    ...
    if not results:
        ...
        print(
            "\ngaps: BSCF components with no matching XBRL tag, counted as zero "
            "(sti/lti = short/long-term investments, cvt_cur/cvt_nc = convertible "
            "notes, other_ltl = other long-term liabilities)."
        ...
        pd.DataFrame(
            [{**r, "bscf_trend": fmt_trend(r["bscf_trend"])} for r in results]
        ).to_csv(csv_path, index=False)
        print(f"\nWrote {csv_path}")

    if skipped:
        out_of_scope = [(t, r) for t, r in skipped if r.startswith(OUT_OF_SCOPE)]
        problems = [(t, r) for t, r in skipped if not r.startswith(OUT_OF_SCOPE)]
        print(f"\n=== Skipped ({len(skipped)}) ===")
        if out_of_scope:
